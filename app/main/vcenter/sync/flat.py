...
def sync_info(platform_id):
    ...
    result_info.setdefault(SyncOperation.PLATFORM, {}).setdefault(platform_id, []).append(
    ...
    for datacenter in connect.rootFolder.childEntity:
        ...
        result_info.setdefault(SyncOperation.DATACENTER, {}).setdefault(platform_id, []).append(datacenter)
...
def _flat_portgroup(result_info, network_portgroups, platform_id):
    ...
    for portgroup in network_portgroups:
        result_info.setdefault(SyncOperation.PORTGROUP, {}).setdefault(platform_id, []).append(portgroup)
...
def _flat_dvswitch(result_info, network_folder, platform_id):
    ...
    for dvs in network_folder:
        if isinstance(dvs, vim.dvs.VmwareDistributedVirtualSwitch):
            result_info.setdefault(SyncOperation.DVSWITCH, {}).setdefault(platform_id, []).append(dvs)
...
def _flat_datastore(result_info, datastores, platform_id):
    ...
    for ds in datastores:
        result_info.setdefault(SyncOperation.DATASTORE, {}).setdefault(platform_id, []).append(ds)
...
def _flat_cluster(result_info, cluster_folder, platform_id, connect):
    ...
    for cluster in cluster_folder:
        ...
        result_info.setdefault(SyncOperation.CLUSTER, {}).setdefault(platform_id, []).append(cluster)
...
def _flat_cluster_resourcepool(result_info, resource_pools, platform_id):
    ...
    for rp in resource_pools:
        ...
        result_info.setdefault(SyncOperation.RESOURCEPOOL, {}).setdefault(platform_id, []).append(rp)
...
def _flat_cluster_host(result_info, hosts, platform_id):
    ...
    for host in hosts:
        ...
        result_info.setdefault(SyncOperation.HOST, {}).setdefault(platform_id, []).append(host)
...
def _flat_cluster_host_vswitch(result_info, vswitchs, platform_id, host):
    ...
    """
    for vs in vswitchs:
        result_info.setdefault(SyncOperation.VSWITCH, {}).setdefault(platform_id, []).append((vs, host))
...
def _flat_cluster_host_vm(result_info, vms, platform_id):
    ...
    """
    for vm in vms:
        result_info.setdefault(SyncOperation.VM, {}).setdefault(platform_id, []).append(vm)
